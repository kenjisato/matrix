...
斜行座標を表示するとわかりやすい。下図は $\lambda = 0.65 + 0.65j$, $\alpha = (1, 0.3), \beta=(0.3, 1)$ に設定したもの。初期値を青軸にとっている。偏角は45度なので、2ステップごとに軸に乗る。奇数ステップ目の位置に注目すると、斜行座標の角度をちょうど半分にした位置にあることがわかる。

![45degree](img/7-trajectory.png)